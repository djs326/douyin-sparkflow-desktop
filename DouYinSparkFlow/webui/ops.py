...
def run_task_now(*, unsent_only=False, failed_only=False, force_all=False, account_refs=None):
    try:
        ...
        log_file = Path(get_app_settings().get("ops_log_file") or default_ops_log_path())
...
def refresh_proxy():
    # 桌面版不包含 Mihomo 代理编排，直接提示不可用。
    logger.info("refresh_proxy is not supported in the desktop build")
    return _empty_result(stderr="desktop build does not include proxy orchestration")


def restart_proxy():
    logger.info("restart_proxy is not supported in the desktop build")
    return _empty_result(stderr="desktop build does not include proxy orchestration")
...
def update_daily_schedule(time_string):
    ...
    if os.name == "nt":
        return subprocess.CompletedProcess(
            args=["update-daily-schedule"],
            returncode=0,
            stdout="schedule saved; in-app scheduler will handle it",
            stderr="",
        )
...
def sync_daily_schedule_from_config():
    ...
    # Windows 桌面版没有 crontab，由应用内调度器负责，无需同步。
    if os.name == "nt":
        return subprocess.CompletedProcess(
            args=["sync-daily-schedule"],
            returncode=0,
            stdout="in-app scheduler; no crontab to sync",
            stderr="",
        )
